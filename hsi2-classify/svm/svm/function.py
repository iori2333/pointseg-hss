...
def train_average(model: SVC, train_loader: DataLoader, sz: int = 11):
    ...
    try:
        ...
        n = len(train_loader)
        X_train, y_train = [], []
        for i, (image, label, hsi, _) in enumerate(train_loader):
...
def async_val(model: SVC, i: int, name: str, label: np.ndarray, image: torch.Tensor, sz: int, out: str):
    b, c, h, w = image.shape
    image = image.squeeze(0).permute(1, 2, 0).numpy()
    label = label.squeeze(0).numpy()
    name = name[0]
    ret = cv2.imread(f'{out}/rgb{name}_gray.png', cv2.IMREAD_GRAYSCALE)
    if ret is None:
        ...
        cv2.imwrite(f'{out}/rgb{name}_gray.png', ret)
...
def validate(model: SVC, test_loader: DataLoader, sz: int, out: str):
    ...
    for i, (image, label, hsi, name) in enumerate(test_loader):
        logging.info(f'validating image {name} {i}/{n}')
        current = async_val(model, i, name, label, hsi, sz, out)
