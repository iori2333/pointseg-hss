...
class HSICity2(Dataset):
    ...
    def _load(self) -> List[str]:
        ...
        hsd_dir = os.path.join(self.root, "hsd")
        for img in os.listdir(img_dir):
            ...
            if self.test_dir is not None and ('rgb%s_gray.png' % name) not in os.listdir(self.test_dir):
                continue
            if name + '.hsd' not in os.listdir(hsd_dir):
                continue
            images.append(name)
        print(len(images))
        return images
    ...
    def __getitem__(self, index):
        ...
        image = cv2.imread(
            os.path.join(self.root, "img", "rgb" + name + ".jpg"), cv2.IMREAD_COLOR
        )
        label = cv2.imread(
            os.path.join(self.root, "gt", "rgb" + name + "_gray.png"), cv2.IMREAD_GRAYSCALE
        )

        h, w, _ = image.shape
        
        if self.use_hsi:
            hsi = self._read_hsd(os.path.join(self.root, "hsd", name + ".hsd"))
